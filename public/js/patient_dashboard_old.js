...
function saveAppointmentDetail(key){
	...
	var hosp_id = $("#hosps_id").val();
	var reanson_for_visit = $('#reanson_for_visit').val();
	var doctor_id = $("#doctors_id").val();
	var site_url = $('#site_url').text();
	$.ajaxSetup({
		headers:{ 'X-CSRF-Token': $('input[name="_token"]').val() }
	});
	var url = site_url+"/patient/save_appointment_details";	
	$.ajax({				
		type: 'POST',
		url: url,		
		data: {"time":time,"doctor_id":doctor_id,"hosp_id":hosp_id,'reanson_for_visit':reanson_for_visit},		
		dataType: 'json',
		success: function (response) {
			if(response['redirect'] == 1){
				swal({
...
function doctorAvailabilityListing(data){
	...
	$.ajax({				
		...
		success: function (response) {
			...
			$("#"+id).find(".availability_listing").html(response);	
			$('.availability_listing_list').show();
			$('#schudule-btn-show').show();
